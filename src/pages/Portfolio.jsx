...
import { 
  ChevronLeft, ChevronRight, Code, Database, Globe, Smartphone, Star, ArrowRight, 
  Github, Linkedin, Mail, ExternalLink, Menu, X, Sun, Moon, ChevronDown, Send,
  MapPin, Phone, Calendar, Award, Users, Coffee, ChevronUp
} from 'lucide-react';
...
const Portfolio = () => {
  ...
  const testimonials = [
    {
      ...
      avatar: "https://images.unsplash.com/photo-1494790108755-2616b2e2e8b4?w=60&h=60&fit=crop&crop=face",
      rating: 5
    },
    {
      name: "Michael Chen",
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${theme.bg} ${theme.text}`}>
      {/* Scroll progress bar */}
      <div className="fixed top-0 left-0 h-1 bg-gradient-to-r from-blue-500 to-cyan-500 z-50" 
           style={{ width: `${scrollProgress}%` }} />

      {/* Modern Floating Navbar */}
      <nav className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-50 transition-all duration-300 ${
        scrollY > 100 ? 'w-11/12 md:w-auto' : 'w-11/12 md:w-auto'
      }`}>
        <div className={`${isDarkMode ? 'bg-gray-900/90' : 'bg-white/90'} backdrop-blur-lg rounded-2xl border ${theme.border} shadow-xl px-6 py-3`}>
          <div className="flex justify-between items-center">
            <div className="text-xl font-bold bg-gradient-to-r from-blue-500 to-cyan-500 bg-clip-text text-transparent md:hidden">
              Alex Rivera
            </div>
            ...
            <div className="hidden md:flex items-center space-x-8">
              ...
              <div className="relative">
                ...
                {activeDropdown === 'services' && (
                  <div className={`absolute top-full mt-2 w-64 ${theme.cardBg} rounded-xl border ${theme.border} shadow-xl p-2`}>
          ...
          {isMenuOpen && (
            <div className="md:hidden mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <div className="flex flex-col space-y-3">
                <button onClick={() => scrollToSection('home')} className="text-left hover:text-blue-500 transition-colors">Home</button>
                <button onClick={() => scrollToSection('about')} className="text-left hover:text-blue-500 transition-colors">About</button>
                <button onClick={() => scrollToSection('projects')} className="text-left hover:text-blue-500 transition-colors">Projects</button>
                <button onClick={() => scrollToSection('contact')} className="text-left hover:text-blue-500 transition-colors">Contact</button>
              </div>
            </div>
          )}
        </div>
      ...
      <section 
        id="home" 
        className={`min-h-screen flex items-center justify-center pt-20 px-6 transition-opacity duration-500 ${visibleSections.has('home') ? 'opacity-100' : 'opacity-0 translate-y-10'}`}
      >
        <div className="container mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-bold mb-4 bg-gradient-to-r from-blue-500 to-cyan-500 bg-clip-text text-transparent">
            Alex Rivera
          </h1>
          ...
          <p className={`text-lg ${theme.textSecondary} mb-8 max-w-2xl mx-auto`}>
            I craft exceptional digital experiences that drive business growth. From concept to deployment, I deliver premium solutions.
          </p>
        ...
        <div className="container mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {stats.map((stat, index) => (
              <div key={index} className={`${theme.cardBg} border ${theme.border} rounded-xl p-6 text-center hover:shadow-lg transition-all duration-300 hover:scale-105`}>
      ...
      <section 
        id="about" 
        className={`py-16 px-6 md:px-16 transition-opacity duration-500 ${visibleSections.has('about') ? 'opacity-100' : 'opacity-0 translate-y-10'}`}
      >
        <div className="container mx-auto">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold mb-6 bg-gradient-to-r from-blue-500 to-cyan-500 bg-clip-text text-transparent">
                About Me
              </h2>
              <p className={`${theme.textSecondary} mb-6 leading-relaxed`}>
                With over 5 years of experience in full-stack development, I specialize in creating scalable, user-friendly applications that solve real business problems. I'm passionate about clean code, modern architectures, and delivering exceptional user experiences.
              </p>
              <div className="flex flex-wrap gap-3">
                {['React', 'Node.js', 'Python', 'TypeScript', 'AWS', 'MongoDB'].map((tech, index) => (
                  <span key={index} className="px-3 py-1 bg-blue-500/10 text-blue-500 rounded-full text-sm border border-blue-500/20">
                    {tech}
                  </span>
                ))}
              </div>
      ...
      <section 
        id="projects" 
        className={`py-16 px-6 transition-opacity duration-500 ${visibleSections.has('projects') ? 'opacity-100' : 'opacity-0 translate-y-10'}`}
      >
        <div className="container mx-auto">
          ...
          <div className="relative max-w-4xl mx-auto">
                      ...
                      <div className="p-8">
                        ...
                        <div className="flex flex-wrap gap-2 mb-6">
                          {project.tech.map((tech, i) => (
                            <span key={i} className="px-3 py-1 bg-blue-500/10 text-blue-500 rounded-full text-sm border border-blue-500/20">
                              {tech}
                            </span>
                          ))}
                        </div>
            ...
            <button 
              ...
              className={`absolute left-4 top-1/2 transform -translate-y-1/2 ${theme.cardBg} border ${theme.border} p-2 rounded-full hover:bg-blue-500/10 transition-all`}
            >
            ...
            <button 
              ...
              className={`absolute right-4 top-1/2 transform -translate-y-1/2 ${theme.cardBg} border ${theme.border} p-2 rounded-full hover:bg-blue-500/10 transition-all`}
            >
              ...
              {projects.map((_, index) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full transition-all duration-300 ${
                    index === currentProject ? 'bg-blue-500 w-6' : `${isDarkMode ? 'bg-gray-600' : 'bg-gray-300'} hover:bg-gray-500`
                  }`}
                />
              ))}
            </div>
        ...
        <div className="container mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {testimonials.map((testimonial, index) => (
              <div key={index} className={`${theme.cardBg} border ${theme.border} rounded-xl p-6 hover:shadow-lg transition-all duration-300`}>
      ...
      <section 
        id="contact" 
        className={`py-16 px-6 transition-opacity duration-500 ${visibleSections.has('contact') ? 'opacity-100' : 'opacity-0 translate-y-10'}`}
      >
          ...
          <div className="grid md:grid-cols-2 gap-12">
            ...
            <div>
              ...
              <div className="flex space-x-4 mt-8">
                <a href="#" className={`${theme.cardBg} border ${theme.border} p-3 rounded-full hover:bg-blue-500/10 transition-all`}>
      ...
      {/* Footer */}
      <footer id="footer" className={`py-8 border-t ${theme.border} transition-opacity duration-500 ${visibleSections.has('footer') ? 'opacity-100' : 'opacity-0 translate-y-10'}`}>
        <div className="container mx-auto px-6 text-center">
          <p className={theme.textSecondary}>
            © 2025 Alex Rivera. Crafting exceptional digital experiences.
          </p>
        </div>
      </footer>
    </div>
